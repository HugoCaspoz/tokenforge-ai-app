...
export default function AirdropTool({ tokenAddress, selectedAddresses }: AirdropToolProps) {
    ...
    const handleAirdrop = () => {
        ...
        try {
            ...
            console.error(e);
            toast.error(t('tokenDetail.community.airdrop.error'));
        }
    };

    if (isSuccess) {
        toast.success(t('tokenDetail.community.airdrop.success'));
    }

    return (
        <div className="bg-purple-900/20 border border-purple-500/30 rounded-xl p-6 mb-8">
            <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
                {t('tokenDetail.community.airdrop.title')}
            </h3>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label className="block text-sm font-medium text-gray-400 mb-2">
                        {t('tokenDetail.community.airdrop.selectedUsers')}
                    </label>
                    <div className="bg-gray-900/50 rounded-lg p-3 border border-gray-700 text-white font-mono">
                        {t('tokenDetail.community.airdrop.usersCount').replace('{count}', String(selectedAddresses.length))}
                    </div>
                </div>

                <div>
                    <label className="block text-sm font-medium text-gray-400 mb-2">
                        {t('tokenDetail.community.airdrop.amountPerUser')}
                    </label>
                    <div className="relative">
                        ...
                        <div className="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                            <span className="text-gray-400 text-xs">{t('tokenDetail.community.airdrop.tokens')}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="mt-6">
                ...
                >
                    {isPending || isConfirming ? t('tokenDetail.community.airdrop.sending') : t('tokenDetail.community.airdrop.sendButton').replace('{count}', String(selectedAddresses.length))}
                </button>
                <p className="text-xs text-gray-500 mt-2 text-center">
                    {t('tokenDetail.community.airdrop.disclaimer').replace('{amount}', amountPerUser || '0')}
                </p>
            </div>
        </div>
    );
