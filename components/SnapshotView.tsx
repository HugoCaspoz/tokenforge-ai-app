...
export default function SnapshotView({ holders, loading, onSelectForAirdrop }: SnapshotViewProps) {
    ...
    if (loading) {
        return <div className="text-center py-10 text-gray-400">{t('tokenDetail.community.snapshotTable.loading')}</div>;
    }

    if (holders.length === 0) {
        return <div className="text-center py-10 text-gray-400">{t('tokenDetail.community.snapshotTable.noHolders')}</div>;
    }

    return (
        <div className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden">
            <div className="p-4 border-b border-gray-700 flex justify-between items-center">
                <h3 className="font-bold text-white">{t('tokenDetail.community.snapshotTable.title').replace('{count}', String(holders.length))}</h3>
                <button
                    onClick={() => {
                ...
                >
                    {t('tokenDetail.community.snapshotTable.export')}
                </button>
            </div>
                ...
                <table className="w-full text-left text-sm text-gray-400">
                    <thead className="bg-gray-900/50 text-gray-200 uppercase text-xs">
                        <tr>
                            ...
                            <th className="p-4">{t('tokenDetail.community.snapshotTable.rank')}</th>
                            <th className="p-4">{t('tokenDetail.community.snapshotTable.address')}</th>
                            <th className="p-4 text-right">{t('tokenDetail.community.snapshotTable.balance')}</th>
                            <th className="p-4 text-right">{t('tokenDetail.community.snapshotTable.percentage')}</th>
                        </tr>
                    </thead>
